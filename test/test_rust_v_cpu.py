...
def helper_test_speed(f, *args, device="CPU"):
    ...
    for i in range(CNT):
        ...
        args = [(x + 1).realize() if isinstance(x, Tensor) else x for x in args]
        args = [(x - 1).realize() if isinstance(x, Tensor) else x for x in args]
    ...
    return ret.numpy() if isinstance(ret, Tensor) else ret, np.min(ets)

def helper_test_generic_square(name, N, f_cpu, f_rust, onearg=False):
    ...
    helper_test_generic(f"{name:30s} {N:5d}x{N:5d}", f_cpu, (cpu_a, cpu_b), f_rust, (rust_a, rust_b), "CPU", "RUST")
...
def helper_test_generic(name, f_cpu, cpu_args, f_rust, rust_args, cpu_device, rust_device):
    global prefix
    val_cpu, et_cpu = helper_test_speed(f_cpu, *cpu_args, device=cpu_device)
    val_rust, et_rust = helper_test_speed(f_rust, *rust_args, device=rust_device)

    desc = "faster" if et_cpu > et_rust else "slower"
    flops = save_ops * 1e-6
    mem = save_mem * 1e-6
    print(("\r" if not CI else "") + f"{name:42s} {et_cpu:7.2f} ms ({flops/et_cpu:9.2f} GFLOPS {mem/et_cpu:7.2f} GB/s) in CPU, {et_rust:7.2f} ms ({flops/et_rust:9.2f} GFLOPS {mem/et_rust:7.2f} GB/s) in RUST, {colorize_float(et_rust/et_cpu)} {desc} {flops:10.2f} MOPS {mem:8.2f} MB")
...
class TestSpeed(unittest.TestCase):
    ...
    def test_cat(self):
        np.random.seed(0)  # Ensure reproducibility
        data_a = np.random.rand(2048, 2048) - 0.5
        data_b = np.random.rand(2048, 2048) - 0.5
        ...
        helper_test_generic(
            f"cat_0 {2048:5d}x{2048:5d}",
            lambda x, y: x.cat(y, dim=0),
            (cpu_a, cpu_b),
            TinyJit(lambda x, y: x.cat(y, dim=0)),
            (rust_a, rust_b),
            "CPU",
            "RUST"
        )
        helper_test_generic(
            f"cat_1 {2048:5d}x{2048:5d}",
            lambda x, y: x.cat(y, dim=1),
            (cpu_a, cpu_b),
            TinyJit(lambda x, y: x.cat(y, dim=1)),
            (rust_a, rust_b),
            "CPU",
            "RUST"
        )
